...
const Sidebar_buttons = ({ title, selected, onClick, icon }) => {
     ...
     const buttonStyle = isSelected
          ? "md:pr-0 flex justify-start items-center space-x-2 cursor-pointer bg-[#FF4C30] rounded-lg pr-3 transition ease-out duration-200"
          : "md:pr-0 flex justify-start items-center space-x-2 cursor-pointer transition ease-out duration-200 hover:bg-[#FF4C30] hover:bg-opacity-50 rounded-lg pr-3 hover:font-bold";
     ...
     return (
          ...
          <Link to={title} className={buttonStyle} id='father' onClick={() => onClick(title)}>
               <div className="h-12 pl-4 pr-2 rounded-lg justify-center items-center inline-flex">
